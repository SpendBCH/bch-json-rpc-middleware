...
function createWalletMiddleware(opts = {}) {
  ...
  const getSlpAccounts = opts.getSlpAccounts ? opts.getSlpAccounts : opts.getAccounts
  ...
  return createScaffoldMiddleware({
    ...
    // slp account lookups
    'slp_accounts': createAsyncMiddleware(lookupSlpAccounts),
    'slp_coinbase': createAsyncMiddleware(lookupSlpDefaultAccount),
    // tx signatures
  ...
  async function lookupSlpAccounts(req, res) {
    if (!getSlpAccounts) throw new Error('WalletMiddleware - opts.getSlpAccounts not provided')
    const accounts = await getSlpAccounts(req)
    res.result = accounts
  }

  async function lookupSlpDefaultAccount(req, res) {
    if (!getSlpAccounts) throw new Error('WalletMiddleware - opts.getSlpAccounts not provided')
    const accounts = await getSlpAccounts(req)
    res.result = accounts[0] || null
  }
  ...
  async function validateSender(address, req) {
    ...
    if (!getAccounts) throw new Error('WalletMiddleware - opts.getAccounts not provided')
    const accounts = await getAccounts(req)
    const slpAccounts = await getSlpAccounts(req)
    const allAccounts = accounts.concat(slpAccounts)
    const normalizedAccounts = allAccounts.map(address => address.toLowerCase())
